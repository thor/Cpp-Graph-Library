...
Graph::Graph(const std::string& inputFileName) {
    ...
    if (line ==
        ...
        directed = true;
    } else if (line == specialChar + std::string("PERSISTANT GRAPH: UNDIRECTED "
    ...
    while (getline(file, line)) {
        ...
        std::cout << sourceNode << " " << targetNode << " " << weight
                  << std::endl;
...
bool Graph::addNode(std::string name) { return addNode(1, name); }
...
bool Graph::addEdge(std::string fromNode, std::string toNode, double weight) {
    ...
    if (nodeMap.find(fromNode) == nodeMap.end()) {
        return false;
    }
    if (nodeMap.find(toNode) == nodeMap.end()) {
        return false;
    }
...
bool Graph::deleteEdge(std::string fromNode, std::string toNode,
    ...
    if (!directed) {
        std::unordered_map<std::string, std::multiset<double>>&
            neighborMapRef1 = *(nodeMap[toNode]->getMapPtr());
...
bool Graph::weaklyConnected() const {
    ...
    // Replace all directed edges with undirected edges (ie for all edges
    // <A,B,w> add <B,A,w>)
    std::vector<std::tuple<std::string, std::string, double>> edgeVec =
...
Graph::neighborDistMin(std::string sourceNode) {
...
std::vector<std::pair<std::string, double>>
Graph::neighborDistMax(std::string sourceNode) {
    std::vector<std::pair<std::string, double>> returnVec;
...
bool Graph::deleteNeighbors(std::string sourceNode) {
    if (nodeMap.find(sourceNode) == nodeMap.end()) {
        return false;
    }
...
std::unordered_set<std::string> Graph::explore(std::string sourceNode) {
    std::unordered_set<std::string>
        reachable; // Will contain all nodes reachable from the passed Node
...
/// sourceNode (uses BFS)
std::vector<std::pair<std::string, double>>
Graph::reachableDists(std::string sourceNode) {
...
std::vector<std::string> Graph::BFS(std::string sourceNode,
...
std::vector<std::string> Graph::DFS(std::string sourceNode,
                                    std::string targetNode) {
    // If either Node DNE, return an empty vector
    std::vector<std::string> pathVec;
    if (nodeMap.find(sourceNode) == nodeMap.end()) {
...
void Graph::DFShelper(std::string currentNode, std::string targetNode,
                      std::unordered_map<std::string, std::string>& prevMap) {
    ...
    for (std::string neighbor : neighbors) {
        // If this neighbor has not been visited, add it to the prevMap and
        // recurse on it
...
std::vector<std::string> Graph::Dijktras(std::string sourceNode,
                                         std::string targetNode) {
    double infinity = std::numeric_limits<double>::max(); // Simulated infinity
    std::unordered_map<std::string, double>
    ...
    // For all Nodes N, set their distance from source to infinity, all prevs
    // are null
    for (auto iter : nodeMap) {
    ...
    // .second is the name C++ will use the first value of pair as the
    // comparison
    ...
    while (!minHeap.empty()) {
        ...
        for (std::string N : neighborsCurr) {
            std::unordered_map<std::string, std::multiset<double>>*
                neighborMapPtr = nodeMap[currNode]->getMapPtr();
            double distanceToN =
                dist[currNode] + *((*neighborMapPtr)[N]).begin();
...
std::unordered_map<std::string, double>
Graph::Dijktras(std::string sourceNode) {
    double infinity = std::numeric_limits<double>::max(); // Simulated infinity
    std::unordered_map<std::string, double>
        dist; // Holds the shortest distance to each Node from targetNode
    std::unordered_map<std::string, std::string>
...
Graph::BellmanFord(std::string sourceNode) {
    double infinity = std::numeric_limits<double>::max(); // Simulated infinity
    ...
    std::unordered_map<std::string, double>
        Dist; // Holds the shortest distance to each Node from sourceNode
    std::unordered_map<std::string, std::string> Prev; // Holds the previous
                                                       // Node
    for (auto iter : nodeMap) {
        Dist.emplace(iter.first, infinity);
...
std::unordered_map<std::string, double>
Graph::BellmanFordDist(std::string sourceNode) {
    return std::get<0>(BellmanFord(sourceNode));
}
std::unordered_map<std::string, std::string>
Graph::BellmanFordPrev(std::string sourceNode) {
...
Graph Graph::Prims() {
    // Initialize a tree with a single vertex, chosen arbitrarily from the
    // graph.
    Graph MST;
    if (!connected()) {
        return MST;
    ...
    while (MST.numEdges() !=
           (numNodes() - 1)) { // There are |N-1| Edges in a MST
        for (auto edge : edges) {
            ...
            if ((MST.nodeExists(std::get<0>(edge)) &&
                ...
                MST.addEdge(std::get<0>(edge), std::get<1>(edge),
                            std::get<2>(edge));
                break;
            }
        }
    ...
    for (auto iter : nodeMap) {
        ...
        for (auto iter1 : *(K->getMapPtr())) {
            ...
            for (double i : tempSet) {
                ...
                std::tuple<std::string, std::string, double> tempTuple(
                    nodeA, nodeB, i);
    ...
    if (!directed) {
        ...
        for (auto edge : edgeVec) {
            ...
            std::tuple<std::string, std::string, double> deleteMe(nodeB, nodeA,
                                                                  weight);
            if (nodeA > nodeB) // Prevents deleting both duplicates, we just
                               // want to delete one to leave a unique edge.
...
int Graph::numNodes() { return nodeMap.size(); }

int Graph::numEdges() { return getEdges().size(); }
...
bool Graph::saveGraph(std::string outputFileName) {
    ...
    if (directed) {
        output << specialChar
               << "PERSISTANT GRAPH: DIRECTED (Do not edit this line)"
               << std::endl;
    } else {
        output << specialChar
    ...
    for (auto iter : nodeMap) {
        output << iter.first << separator << nodeMap[iter.first]->getData()
               << std::endl;
    }
    ...
    for (auto tuple : getEdges()) {
        output << std::get<0>(tuple) << separator << std::get<1>(tuple)
               << separator << std::get<2>(tuple) << std::endl;
    }
