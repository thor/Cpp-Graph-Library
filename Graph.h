...
class Graph {
  ...
  public:
    ...
    bool addEdge(std::string fromNode, std::string toNode); // Default weight
                                                            // '1'
    ...
    bool deleteEdge(std::string fromNode,
                    std::string toNode); // Default weight '1'
    ...
    Graph transpose()
        const; // Creates a copy, reverses edges of that copy and returns it.

    // Neighbor Functions
    std::vector<std::string> neighborNames(std::string name);
    std::vector<std::pair<std::string, double>>
    neighborDistMin(std::string name);
    std::vector<std::pair<std::string, double>>
    neighborDistMax(std::string name);
    bool deleteNeighbors(std::string name);

    // Explore Functions
    std::unordered_set<std::string>
    explore(std::string sourceNode); // Returns a set of Nodes reachable from
                                     // the source Node
    void exploreHelper(std::unordered_set<std::string> &visited,
                       std::string name);
    std::vector<std::string>
    reachableNames(std::string sourceNode); // Returns a list of Nodes that are
                                            // reachable from the target
    std::vector<std::pair<std::string, double>>
    reachableDists(std::string sourceNode); // Includes distances
    bool pathCheck(std::string fromNode, std::string toNode);

    // Core Graph Functions
    std::vector<std::string>
    BFS(std::string sourceNode,
        std::string
            targetNode); // Returns the shortest path from source to target
    std::vector<std::string>
    DFS(std::string sourceNode,
        std::string
            targetNode); // Returns the shortest path from source to target
    void DFShelper(std::string sourceNode, std::string targetNode,
                   std::unordered_map<std::string, std::string> &prevMap);
    std::vector<std::string>
    Dijktras(std::string sourceNode,
             std::string
                 targetNode); // Returns the shortest path from source to target
    ...
    BellmanFord(std::string sourceNode);
    std::unordered_map<std::string, double>
    BellmanFordDist(std::string sourceNode); // Returns just the Dist map
    std::unordered_map<std::string, std::string>
    BellmanFordPrev(std::string sourceNode); // Returns just the Prev map
    bool NegativeCycle(); // Does the graph contain a negCycle? Warning!:
    ...
    std::string getInfo(); // Returns a list of all Nodes along with their
                           // Edges.
    std::vector<std::tuple<std::string, std::string, double>>
    getEdges() const; // Returns a vector of Edges, where Edges are represented
                      // with a 3-tuple (nodeA,nodeB,weight)
    std::vector<std::tuple<std::string, std::string, double>>
    getEdgesAscending() const;
    std::vector<std::tuple<std::string, std::string, double>>
    getEdgesDescending() const;
